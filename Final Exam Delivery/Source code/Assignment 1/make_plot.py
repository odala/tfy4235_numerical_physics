...
data12nm = np.loadtxt('drift_velocities_12nm.txt')
tau12nm = data12nm[:, 0]
avg_vd12nm = data12nm[:, 1]
std_dev12nm = data12nm[:, 2]
data36nm = np.loadtxt('drift_velocities_36nm.txt')
tau36nm = data36nm[:, 0]
avg_vd36nm = data36nm[:, 1]*0.5
std_dev36nm = data36nm[:, 2]
fmax_x, fmax_y, fmax_std, fmin_x, fmin_y, fmin_std = plot_w_std_dev([tau12nm], [1e6*avg_vd12nm], [1e6*std_dev12nm], r'Period of flash, $\tau$ [s]', r'Average drift velocity, $\left< v_d \right>$ [$\mathrm{\mu}$m/s]', 'fig/drift_velocities_12.png', ['b.'], errorbar = 'filled')
fmax_x, fmax_y, fmax_std, fmin_x, fmin_y, fmin_std = plot_w_std_dev([tau36nm], [1e6*avg_vd36nm], [1e6*std_dev36nm], r'Period of flash, $\tau$ [s]', r'Average drift velocity, $\left< v_d \right>$ [$\mathrm{\mu}$m/s]', 'fig/drift_velocities_36.png', ['r.'], errorbar = 'filled')
