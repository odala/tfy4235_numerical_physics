...
def time_until_wall_collision(particle, t):
    ...
    if (t - particle.last) < tc:
        xi_used = 1.0
    else:
        xi_used = xi
    if timex < timey:
        return timex, np.array([-xi_used*particle.vx, xi_used*particle.vy])
    else:
        return timey, np.array([xi_used*particle.vx, -xi_used*particle.vy])
...
def time_until_collision(particle1, particle2, t):
    ...
    if np.dot(dv,dx) >= 0:
        ...
        if (t - particle1.last) < tc or (t - particle2.last) < tc:
            xi_used = 1.0
        else:
            xi_used = xi
        velocity1 = particle1.velocity() + ((1.0+xi_used)*particle2.mass/(particle1.mass+particle2.mass)*np.dot(dv,dx2)/R**2)*dx2
        velocity2 = particle2.velocity() - ((1.0+xi_used)*particle1.mass/(particle1.mass+particle2.mass)*np.dot(dv,dx2)/R**2)*dx2
...
def run(particles, start_time, stop_colcount, plotter=False):
    ...
    for i in range (len(particles)):
        ...
        for j in range(i+1, len(particles)):
            dt, v1, v2 = time_until_collision(particles[i], particles[j], t0)
            if dt < float('inf'):
                temp = t0 + dt
        ...
        # and store the collision times.
        dt, v1 = time_until_wall_collision(particles[i], t0)
        if dt < float('inf'):
            temp = t0 + dt
    ...
    #for loop in range(1):
        ...
        #     colcount if it was a particle-particle collision
        #     calculate new velocities and save collision time.
        for i in range(len(collision.involved)):
            if (len(collision.involved) > 1):
                particles[collision.involved[i]].increment_colcount()
            particles[collision.involved[i]].set_velocity(collision.new_velocities[i])
            particles[collision.involved[i]].last = t0
