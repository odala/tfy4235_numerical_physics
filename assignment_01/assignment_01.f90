program biased_brownian_motion
	...
	real(wp) 							:: zeta, gama, D, omega !// dynamic viscosity, friction constant, kT/dU, dU/gama/L**2
	real(wp)							:: dt
	...
	real(wp), parameter     :: std_dU       = 0.26_wp   ! 10*kT	!80.0_wp		! eV
    real(wp), parameter     :: std_alfa    	= 0.2_wp
    real(wp), parameter     :: std_tau		= 4.0_wp
    real(wp), parameter     :: std_L        = 20E-6_wp
    real(wp), parameter     :: std_zeta     = 1E-3_wp
    real(wp), parameter     :: std_kT	    = 0.026_wp		! eV
    real(wp), parameter     :: std_x0       = 0.0_wp
    real(wp), parameter     :: std_N       	= 1.0_wp
	...
	!// Converts to reduced units
	!// introduce new t = old t * omega
	...
	if ( 1/alfa >= 1/(1-alfa) ) then
		dt = (-sqrt(8.0_wp*D) + sqrt(8.0_wp*D**2.0_wp + 0.1_wp)) * alfa
	else 
		dt = (-sqrt(8.0_wp*D) + sqrt(8.0_wp*D**2.0_wp + 0.1_wp*alfa / (1.0_wp-alfa))) * (1.0_wp-alfa)
	end if
	
	write(*,*) dt
	
	!// Start the diffusion process for the N particles
	write(*, *, advance='no') 'How many particles do you want in this potential? (', std_N, '):'
	call input_w_default(N, std_N)
